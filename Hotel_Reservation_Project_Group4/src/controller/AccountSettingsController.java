...
public class AccountSettingsController implements Initializable {
	...
	@FXML
	private Label nameErrorText; // first name error only
	...
	public void handleSubmitChanges( ActionEvent event ) throws IOException {
		...
		// Initialize data accessor via link to DB
			...
			if(!patternMatches(textfield1.getText(), nameRegexPattern) || !patternMatches(textfield2.getText(), nameRegexPattern)){ 
				nameErrorText.setText("Please only enter letters and apostrophes. Titles like 'the third' should be entered as 'III'.");
				nameErrorText.setStyle("-fx-font-weight: bold");
			...
			if(!patternMatches(textfield3.getText(), emailRegexPattern)){ 
			...
			if(!patternMatches(textfield4.getText(), phoneNumRegexPattern)){ 
			...
			if(!passwordfield1.getText().equals(LoginController.curUser.getPassW())){
